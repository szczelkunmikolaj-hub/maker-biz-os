...
export function GlobalStatusBar() {
  ...
  const stats = useMemo(() => {
    ...
    // Threshold: 500g remaining or no purchases logged at all (warn only when there's data)
    const lowMaterial = totalGramsPurchased > 0 && remainingStock < 500;

    // Next deadline: nearest upcoming dueDate among unfinished projects
    ...
    <div className="h-12 border-b bg-card/60 backdrop-blur-sm px-4 flex items-center gap-4 overflow-x-auto shrink-0 text-xs">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <span className="font-semibold text-foreground tabular-nums">
          {stats.printerCount > 0
            ? `${stats.printersBusy} / ${stats.printerCount} busy`
            : `${stats.activeJobs} running`}
        </span>
      </div>
      ...
      {stats.lowMaterial ? (
        <div className="flex items-center gap-1.5 shrink-0 rounded-full border border-warning/40 bg-warning/10 px-2 py-0.5 text-warning">
          ...
          <span className="font-medium">Low filament: {Math.round(stats.remainingStock)}g left</span>
        </div>
      ) : (
      ...
      <div className="flex items-center gap-1.5 min-w-0 ml-auto">
        ...
        {nextDeadlineLabel ? (
          ...
          <span className="text-muted-foreground">No upcoming deadlines</span>
        )}
      </div>
    </div>
  );
